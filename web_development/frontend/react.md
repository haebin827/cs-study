https://fullstackopen.com/en/about
 
로드맵:
https://roadmap.sh/react
https://legacy.reactjs.org/docs/getting-started.html
https://legacy.reactjs.org/docs/getting-started.html
https://www.bezkoder.com/category/react/page/2/
https://www.inflearn.com/course/%ED%95%9C%EC%9E%85-%EB%A6%AC%EC%95%A1%ED%8A%B8
 https://brunch.co.kr/@princox/235
http://mynamesieun.github.io/javascript/ES6-%EB%AC%B8%EB%B2%95-%EC%A0%95%EB%A6%AC/
https://www.reddit.com/r/reactjs/comments/17h4whn/react_roadmap_with_explanations_and_resources_all/?rdt=61274
벤치마킹:
https://wwit.design/
https://uxarchive.com/
 
학습:
https://www.bezkoder.com/react-query-axios/
https://www.bezkoder.com/react-fetch-example/
https://www.bezkoder.com/react-axios-example/
https://www.bezkoder.com/react-hooks-crud-axios-api/
https://www.bezkoder.com/react-pagination-material-ui/
https://www.bezkoder.com/react-pagination-hooks/
https://www.bezkoder.com/react-custom-hook/
https://www.bezkoder.com/react-hooks-form-validation-example-formik/
https://www.bezkoder.com/react-hook-form-material-ui-validation/
https://www.bezkoder.com/react-table-pagination-server-side/
https://www.bezkoder.com/react-material-ui-examples-crud/
 
[React.js 란?]
Meta (Facebook)이 개발한 오픈소스 Javascript 라이브러리로, 대규모 웹 서비스의 UI를 더 편하게 개발하기 위해 만들어진 기술.

![React](web_development/frontend/react.png)
  
[기술적 특징]
컴포넌트를 기반으로 UI를 표현
화면 업데이트 구현이 쉬움 => 선연형 프로그래밍 (불필요한 과정 생략하고 목적만 간결히 명시하는 방식)을 사용하기 때문!
반댓말은 명령형 프로그래밍 (목적을 이루기 위한 모든 일련의 과정을 설명하는 방식)
화면 업데이트가 빠르게 처리됨 => DOM을 일일히 수정하지 않기 때문에 (Virtual DOM으로, 업데이트들이 다 모이면 한번에 짧고 굵게 DOM을 수정함)

 
[구동원리]
React App에는 사실 Web Server가 내장되어있음!
=> npm run dev는 react server를 가동시키기 위한 명령어.
